...
const createBook = async ({
  ...
  try {
    const result = await dbConnection.query(
      ...
      [title, publisheddate, authorid, summary]
    );
...
const updateBook = async ({
  ...
  const result = await dbConnection.query(
    "UPDATE public.books SET title=$1, publisheddate=$2, authorid=$3, summary=$4 WHERE id=$5;",
    [title, publisheddate, authorid, summary, id]
  );
...
const getBookByAuthor = async (authorid: string) => {
  try {
    const result = await dbConnection.query(
      "SELECT * FROM public.books WHERE authorid = $1",
      [authorid]
    );
    const books = [];
